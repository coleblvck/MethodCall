...
fun VolumeDialogContent(
    ...
    val streamType = remember { mutableStateOf(params?.getOrNull(0) ?: "RING") }
    val volume = remember { mutableStateOf(params?.getOrNull(1)?.toIntOrNull() ?: 50) }
    val streamTypes = listOf("RING", "MEDIA", "ALARM", "NOTIFICATION")

    fun saveConfig(phone: String) {
        val args = mutableListOf(streamType.value, volume.value.toString())
        chainToolBox.createOrReplaceChainItem(
            phoneNumber = phone,
            method = MethodType.SET_VOLUME,
            params = args,
    ...
    Column(
        ...
        Row(
            ...
            streamTypes.forEach { type ->
                FilterChip(
                    selected = streamType.value == type,
                    onClick = { streamType.value = type },
                    label = { Text(type) },
        ...
        Slider(
            ...
            onValueChange = { volume.value = it.toInt() },
            valueRange = 0f..100f,
            steps = 99,
            modifier = Modifier.fillMaxWidth()
        )
